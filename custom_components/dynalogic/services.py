"""Services for the Dynalogic parcel tracker integration.

`dynalogic.track_parcel` / `dynalogic.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Dynalogic hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Dynalogic is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Dynalogic services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Dynalogic order number"
            )
        entry = _resolve_entry(hass)

        # The postcode is only validated for shape here, not against the
        # carrier: a service call is often an automation reacting to a mail,
        # and failing it on a parcel the carrier has not registered *yet* would
        # lose the code entirely. A pair that never resolves shows up as a
        # parcel stuck on `unknown`.
        ...
        if not valid_postcode(postal_code):
            raise ServiceValidationError(
                f"'{postal_code}' is not a valid Dutch postcode"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Dynalogic services (single-entry integration, so on unload)."""
