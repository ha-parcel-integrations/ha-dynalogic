"""Dynalogic parcel tracker custom component for Home Assistant."""
...
from .api import DynalogicApiClient
from .const import PLATFORMS
from .coordinator import DynalogicCoordinator, _refresh_interval
...
@dataclass
class DynalogicData:
    """Runtime data attached to the Dynalogic config entry."""

    client: DynalogicApiClient
    coordinator: DynalogicCoordinator


type DynalogicConfigEntry = ConfigEntry[DynalogicData]


async def async_setup_entry(hass: HomeAssistant, entry: DynalogicConfigEntry) -> bool:
    """Set up Dynalogic from a config entry."""
    # No auth: Dynalogic tracking is public, so the HA-managed session is fine.
    ...
    entry.runtime_data = DynalogicData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: DynalogicConfigEntry) -> bool:
    """Unload the Dynalogic config entry."""
