...
*Dynamic polling* — no delivery-window field is known in this carrier's
payload, so `planned_from` is always absent: the "no `planned_from`" branch is
the only one real data has ever taken.
