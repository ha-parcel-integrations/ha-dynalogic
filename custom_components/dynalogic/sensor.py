"""Sensor platform for the Dynalogic parcel tracker integration."""
...
from .coordinator import DynalogicCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: DynalogicConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Dynalogic sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        DynalogicIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            DynalogicParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(DynalogicNextDeliverySensor(coordinator, entry))
    entities.append(DynalogicDeliveredParcelsSensor(coordinator, entry))
    entities.append(DynalogicLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class DynalogicIncomingParcelsSensor(
    CoordinatorEntity[DynalogicCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: DynalogicCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                DynalogicParcelSensor(self.coordinator, self._entry, barcode)
...
class DynalogicParcelSensor(CoordinatorEntity[DynalogicCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Dynalogic parcel."""
    ...
    def __init__(
        self, coordinator: DynalogicCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class DynalogicNextDeliverySensor(
    CoordinatorEntity[DynalogicCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: DynalogicCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class DynalogicDeliveredParcelsSensor(
    CoordinatorEntity[DynalogicCoordinator], SensorEntity
):
    """Recently delivered tracked Dynalogic parcels."""
...
class DynalogicLastUpdateSensor(
    CoordinatorEntity[DynalogicCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Dynalogic was last polled successfully."""
