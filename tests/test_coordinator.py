"""Tests for the Dynalogic coordinator: fetching, caching and events.
...
from custom_components.dynalogic.coordinator import (
    DynalogicCoordinator,
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    """Step 2 of the default delivery scenario: in the network, not on a van."""
...
async def test_polling_stops_entirely_with_nothing_tracked(hass):
    ...
    client = AsyncMock()
    coordinator = DynalogicCoordinator(hass, client, entry)
...
async def test_polling_stops_when_everything_delivered(hass):
    entry = _entry_with([_parcel(DELIVERED_CODE)])
    entry.add_to_hass(hass)
    client = AsyncMock()
...
async def test_polling_is_hot_for_an_out_for_delivery_parcel(hass):
    entry = _entry_with([_parcel(ACTIVE_CODE)])
    entry.add_to_hass(hass)
    client = AsyncMock()
...
async def test_polling_resumes_when_a_parcel_is_added_back(hass):
    """After a full stop, re-adding a tracked parcel recomputes a real tier."""
    entry = _entry_with([])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = DynalogicCoordinator(hass, client, entry)
    ...
    hass.config_entries.async_update_entry(
        entry, options={**entry.options, CONF_PARCELS: [_parcel(ACTIVE_CODE)]}
    )
    client.async_get_parcel.return_value = active_sample(ACTIVE_CODE)
    await coordinator._async_update_data()
...
async def test_update_merges_multiple_parcels(hass):
    entry = _entry_with([_parcel(ACTIVE_CODE), _parcel(DELIVERED_CODE)])
    entry.add_to_hass(hass)
    client = AsyncMock()
...
async def test_update_not_found_shows_pending_placeholder(hass):
    entry = _entry_with([_parcel(OTHER_CODE)])
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = None  # 404
    coordinator = DynalogicCoordinator(hass, client, entry)
...
async def test_update_does_not_warn_about_a_pending_placeholder(hass, caplog):
    """The placeholder is ours, not a response — it has no shape to complain
    about, and a nagging warning every poll would train users to ignore them."""
    entry = _entry_with([_parcel(OTHER_CODE)])
    entry.add_to_hass(hass)
    client = AsyncMock()
    ...
    assert "OrderData" not in caplog.text


async def test_update_checks_the_shape_of_a_real_response(hass, caplog):
    """Pre-1.0: an unseen payload must announce what we do not account for."""
    entry = _entry_with([_parcel(ACTIVE_CODE)])
    entry.add_to_hass(hass)
    sample = active_sample()
    sample["DriverName"] = "Jan de Vries"
    client = AsyncMock()
    ...
    assert "DriverName" in caplog.text
    assert "Jan de Vries" not in caplog.text  # keys, never values
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = DynalogicApiError("HTTP 500")
...
async def test_update_backfills_missing_tracking_number(hass):
    ...
    entry.add_to_hass(hass)
    sample = active_sample()
    del sample["TrackAndTraceNumber"]
    client = AsyncMock()
    client.async_get_parcel.return_value = sample
    coordinator = DynalogicCoordinator(hass, client, entry)
...
async def test_update_prunes_cache_for_untracked_parcels(hass):
    ...
    client.async_get_parcel.return_value = delivered_sample()
    coordinator = DynalogicCoordinator(hass, client, entry)
    coordinator._raw_cache["GONE"] = {"TrackAndTraceNumber": "GONE"}
...
async def test_update_fetches_parcels_concurrently(hass):
    ...
    async def _slow_fetch(code, postcode):
...
async def test_delivered_code_skipped_from_fetch(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.side_effect = lambda code, postal_code: (
        active_sample() if code == ACTIVE_CODE else delivered_sample()
    )
    coordinator = DynalogicCoordinator(hass, client, entry)
    ...
    client.async_get_parcel.assert_called_once_with(ACTIVE_CODE, POSTCODE)
...
async def test_no_events_for_parcel_first_seen_delivered(hass):
    ...
    hass.config_entries.async_update_entry(
        ...
        options={
            **entry.options,
            CONF_PARCELS: [_parcel(ACTIVE_CODE), _parcel(DELIVERED_CODE)],
        },
    )
    await coordinator._async_update_data()
    await hass.async_block_till_done()
...
async def test_fires_registered_event_for_new_parcel(hass):
    ...
    hass.config_entries.async_update_entry(
        ...
        options={
            **entry.options,
            CONF_PARCELS: [_parcel(ACTIVE_CODE), _parcel(OTHER_CODE)],
        },
    )
    client.async_get_parcel.side_effect = lambda code, postcode: active_sample(code)
    await coordinator._async_update_data()
...
# The ETA events are driven directly rather than through a payload: Dynalogic
# exposes no delivery-window field that we can name yet, so `normalize_parcel`
# publishes `planned_from`/`planned_to` as None and no sample can move them.
# The event machinery is suite-wide and stays covered, ready for the day a real
# payload names the window.


async def test_fires_delivery_time_changed_event(hass):
    entry = _entry_with([])
    entry.add_to_hass(hass)
    coordinator = DynalogicCoordinator(hass, AsyncMock(), entry)
    coordinator._known_state = {ACTIVE_CODE: ParcelStatus.OUT_FOR_DELIVERY}
    coordinator._known_delivery_times = {
        ACTIVE_CODE: ("2026-04-29T13:00:00+02:00", "2026-04-29T15:00:00+02:00")
    }
        ...
        [
            {
                ...
                "planned_from": "2026-04-29T16:00:00+02:00",
                "planned_to": "2026-04-29T18:00:00+02:00",
            }
        ]
    )
    await hass.async_block_till_done()
