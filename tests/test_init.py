"""Tests for Dynalogic setup and unload."""
...
async def test_setup_and_unload(hass):
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id=DOMAIN,
        options={CONF_PARCELS: [_parcel(ACTIVE_CODE)], CONF_POSTAL_CODE: POSTCODE},
    )
    ...
    incoming = hass.states.get("sensor.dynalogic_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.dynalogic.api.DynalogicApiClient.async_get_parcel",
        new=AsyncMock(side_effect=DynalogicApiError("Dynalogic unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.dynalogic.api.DynalogicApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
