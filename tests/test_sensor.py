"""Tests for Dynalogic sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.dynalogic.const import ParcelStatus
from custom_components.dynalogic.sensor import (
    DynalogicDeliveredParcelsSensor,
    DynalogicIncomingParcelsSensor,
    DynalogicLastUpdateSensor,
    DynalogicNextDeliverySensor,
    DynalogicParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = DynalogicIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = DynalogicParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = DynalogicParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = DynalogicNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = DynalogicNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = DynalogicDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = DynalogicLastUpdateSensor(coordinator, _entry())
